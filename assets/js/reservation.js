...
$(document).ready(function () {
  ...
  (function () {
    ...
    function validatePhone() {
      ...
      const nationalDigits = digitsOnly(phoneInput.value).slice(
        0,
        meta.nationalLen,
      );
      ...
      if (nationalDigits.length !== meta.nationalLen) {
        phoneInput.setCustomValidity(
          `Для страны ${meta.country} введите ${meta.nationalLen} цифр после кода ${meta.code}`,
        );
        return false;
      }
      phoneInput.setCustomValidity("");
  ...
  (function () {
    const BASE_URL = "https://booking-api-irt8.onrender.com";
    const form = document.getElementById("reservation-form");
    const submitBtn = form ? form.querySelector('button[type="submit"]') : null;
    if (!form) return;

    // Будим сервер сразу при загрузке страницы
    ...
    form.addEventListener("submit", async function (ev) {
      ...
      if (phoneInput) {
        const countryOption = document.querySelector(
          `#countryMenu .country-option[data-value="${payload.PhoneCountry || "RU"}"]`,
        );
      ...
      try {
        const resp = await fetch(BASE_URL + "/api/reservations", {
          method: "POST",
        ...
        alert("Сервер недоступен. Попробуйте ещё раз через несколько секунд.");
      } finally {
