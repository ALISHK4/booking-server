...
class AutoSalonApp(tk.Tk):
    ...
    def create_top_panel(self, parent, search_callback):
        ...
        search_btn = tk.Button(
            frame, text="🔍", command=lambda: search_callback(search_var.get())
        )
    ...
    def create_cars_tab(self):
        ...
        self.cars_tree = self.create_treeview(
            tab,
            ["ID", "Brend", "Model", "Year", "Color", "Price", "Vin", "Availability"],
        )
        btn_add, btn_edit, btn_delete, btn_refresh = self.create_top_panel(
            tab, self.search_cars
        )
    ...
    def load_cars(self):
        ...
        cursor.execute(
            "SELECT ID_Car, Brend, Model, [Year], Color, Price, Vin, Availability FROM Cars"
        )
    ...
    def add_car(self):
        try:
            ...
            year = simpledialog.askinteger(
                "Добавление автомобиля", "Год выпуска (Year):"
            )
            ...
            cursor = self.conn.cursor()
            cursor.execute(
                "INSERT INTO Cars (Brend, Model, [Year], Color, Price, Vin, Availability) VALUES (?, ?, ?, ?, ?, ?, True)",
    ...
    def edit_car(self):
        ...
        new_color = simpledialog.askstring(
            "Редактирование автомобиля", "Новый цвет:", initialvalue=item["values"][4]
        )
        if not new_color:
            return
        cursor = self.conn.cursor()
    ...
    def create_clients_tab(self):
        ...
        self.clients_tree = self.create_treeview(
            tab,
            ["ID", "FirstName", "LastName", "Phone", "Email", "Passport", "Address"],
        )
        btn_add, btn_edit, btn_delete, btn_refresh = self.create_top_panel(
            tab, self.search_clients
        )
    ...
    def load_clients(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT ID_Client, FirstName, LastName, Phone, Email, PassportNumber, Address FROM Clients"
        )
        for row in cursor.fetchall():
    ...
    def add_client(self):
        try:
            ...
            cursor = self.conn.cursor()
            cursor.execute(
                "INSERT INTO Clients (FirstName, LastName, Phone, Email, PassportNumber, Address) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def edit_client(self):
        ...
        new_phone = simpledialog.askstring(
            "Редактирование клиента", "Новый телефон:", initialvalue=item["values"][3]
        )
        if not new_phone:
            return
        cursor = self.conn.cursor()
        cursor.execute(
            "UPDATE Clients SET Phone=? WHERE ID_Client=?", (new_phone, client_id)
        )
        self.conn.commit()
        self.load_clients()
    ...
    def create_sales_tab(self):
        ...
        self.sales_tree = self.create_treeview(
            tab, ["ID", "Model", "LastName", "SaleDate", "SalePrice", "Manager"]
        )
        btn_add, btn_edit, btn_delete, btn_refresh = self.create_top_panel(
            tab, self.search_sales
        )
    ...
    def add_sale(self):
        try:
            ...
            sale_date = simpledialog.askstring(
                "Добавление продажи", "Дата продажи (YYYY-MM-DD):"
            )
            ...
            cursor = self.conn.cursor()
            cursor.execute(
                "INSERT INTO Sales (ID_Car, ID_Client, SaleDate, SalePrice, Manager) VALUES (?, ?, ?, ?, ?)",
    ...
    def edit_sale(self):
        ...
        new_manager = simpledialog.askstring(
            "Редактирование продажи", "Новый менеджер:", initialvalue=item["values"][5]
        )
        if not new_manager:
            return
        cursor = self.conn.cursor()
        cursor.execute(
            "UPDATE Sales SET Manager=? WHERE ID_Sale=?", (new_manager, sale_id)
        )
        self.conn.commit()
        self.load_sales()
